...
class MinimaxAgent(FanoronaAgent):
    ...
    def __init__(self, cutoff: Optional[int] = None, heuristic: Optional[Callable[[FanoronaMove], float]] = None):
    ...
    def move(self, env) -> FanoronaMove:
        ...
        for action in root.actions():
            ...
            if result.env.state.turn_to_play == root.env.state.turn_to_play: # same player's move
                value = max(value, max_value(result))
            else:
