    ...
    for pos in Position.pos_range():
            for direction in Direction.dir_range():
                for capture_type in range(3):
                    end_turn = True
                    action = (pos, direction, capture_type, end_turn)
...
def test_capture_exists():
    ...
    env.reset()
    states = TEST_STATES
    env.state.set_from_board_str(states[0])
    assert env.state.capture_exists()
    env.state.set_from_board_str(states[1])
    assert not env.state.capture_exists()
    env.state.set_from_board_str(states[2])
    assert env.state.capture_exists()
...
def test_in_capturing_seq():
    ...
    env.state.set_from_board_str(states[1])
    assert env.state.in_capturing_seq()
    env.state.set_from_board_str(states[2])
    assert not env.state.in_capturing_seq()
