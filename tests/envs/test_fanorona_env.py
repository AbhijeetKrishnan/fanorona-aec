...
def test_step2():
    ...
    assert env.is_valid(action)
    obs, reward, done, info = env.step(action)
    assert env.get_board_string() == 'WWWWWWWWW/WWW1WWWWW/BWBWWBWBW/BBBBB1BBB/BBBBBB1BB B - - 1'
    assert reward == 0
    assert not done
    env.close()
...
def test_end_turn():
    "Test end turn action by making an opening move and testing all possible end turn actions in action space"
    env = gym.make('fanorona-v0')
    env.reset()
    action = (12, 8, 1, 0) # D2 -> E3 approach capture
    env.step(action)
            ...
            for dir in range(9):
                for capture_type in range(3):
                    for end_turn in range(2):
                        ...
                        #print(action, valid)
                        if valid:
                            print(env.get_board_string(), action)
                            valid += 1
                        env.step(action)